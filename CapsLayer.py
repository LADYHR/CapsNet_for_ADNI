...
class CapsLayer(object): # class CapsLayer inherit class object
	...
	def __call__(self, input, kernel_size=None, stride=None):
		if self.layer_type == 'CONV':
			...
			if not self.with_routing:
				# assert input.get_shape() == [cfg.batch_size, 89, 107, 128]
				...
				# assert capsules.get_shape() == [cfg.batch_size, 41, 50, 128]
				capsules = tf.reshape(capsules, (cfg.batch_size, -1, self.vec_len_J, 1))
				# assert capsules.get_shape() == [cfg.batch_size, 32800, 8, 1]
				capsules = squash(capsules)
				assert capsules.get_shape() == [cfg.batch_size, self.caps_num_J, 8, 1]
				return capsules
	...
	def routing(self, input, b_IJ):
		...
		for r_iter in range(cfg.routing_iter_num):
			with tf.variable_scope('iter_'+str(r_iter)):
				c_IJ = tf.nn.softmax(b_IJ, dim=2)
				if r_iter == (cfg.routing_iter_num - 1):
				...
				elif r_iter < (cfg.routing_iter_num - 1):
					s_J = tf.multiply(c_IJ, u_hat_stopped)
					s_J = tf.reduce_sum(s_J, axis=1, keep_dims=True)
					self.v_J = squash(s_J)
					v_J_tiled = tf.tile(self.v_J, [1, self.caps_num_I, 1, 1, 1])
					assert v_J_tiled.get_shape() == [cfg.batch_size, self.caps_num_I, self.caps_num_J, self.vec_len_J, 1]
					u_produce_v = tf.matmul(u_hat_stopped, v_J_tiled, transpose_a=True)
					assert u_produce_v.get_shape() == [cfg.batch_size, self.caps_num_I, self.caps_num_J, 1, 1]
					b_IJ += u_produce_v
		return self.v_J
