...
class CapsNet(object):
	...
	def build_arch(self):
		with tf.variable_scope('Conv1_layer'):
			...
			conv1 = tf.contrib.layers.conv2d(self.X, num_outputs=128, kernel_size=3,
											 stride=1, padding='VALID'_)
			assert conv1.get_shape() == [cfg.batch_size, 89, 89, 128]

		with tf.variable_scope('PrimaryCaps_layer'):
			#  Primary Capsule layer, [batch_size, 41*41*16,8,1]
			primaryCaps = CapsLayer(caps_num=26896, num_outputs=16,vec_len=8,with_routing=False,layer_type='CONV')
			caps1 = primaryCaps(conv1, kernel_size=9, stride=2)
			assert  caps1.get_shape() == [cfg.batch_size, 26896, 8, 1]
			#  Primary Capsule layer2, [batch_size, 17*17*16,8,1]
			primaryCaps2 = CapsLayer(caps_num=4624, num_outputs=16, vec_len=8, with_routing=True, layer_type='CONV')
			caps2 = primaryCaps2(caps1, kernel_size=9, stride=2)
			assert caps2.get_shape() == [cfg.batch_size, 4624, 8, 1]

		with tf.variable_scope('DigitCaps_layer'):
			digitCaps = CapsLayer(caps_num=3, num_outputs=3, vec_len=16, with_routing=True, layer_type='FC')
