...
class CapsNet(object):
    def __init__(self,is_training=True):
        ...
        with self.graph.as_default():
            if is_training:
                self.te_img, self.te_label = get_batch_data(cfg.test_set, cfg.batch_size, cfg.num_threads)
                self.val_img, self.val_label = get_batch_data(cfg.valuation_set, cfg.batch_size, cfg.num_threads)
                self.X, self.labels = get_batch_data(cfg.train_set, cfg.batch_size, cfg.num_threads)
    ...
    def loss(self):
        ...
        self.v_length = tf.sqrt(tf.reduce_sum(tf.square(self.caps3), axis=2, keepdims=True)+epsilon)  # axis=2 ???
